...
public:
    ...
    void searchByDate(const string& searchDate) const {
        bool found = false;
        
        printHeader("BLOOD SUGAR RECORDS FOR " + searchDate);
        cout << setw((termWidth - boxWidth) / 2) << "" << left << setw(12) << "Date" << setw(15) << "Time of Day"
             << right << setw(15) << "Sugar Level" << endl;
        printBorder('|', '-', '|');

        for (int i = 0; i < entryCount; i++) {
            if (entries[i].date == searchDate) {
                found = true;
                cout << setw((termWidth - boxWidth) / 2) << "" << left << setw(12) << entries[i].date
                     << setw(15) << entries[i].timeOfDay
                     << right << setw(15) << entries[i].bloodSugarLevel << " mg/dL" << endl;
        ...
        if (!found) {
            cout << centerText("No records found for " + searchDate) << endl;
        }
    }
...
public:
    ...
    void searchByDate(const string& searchDate) const {
        ...
        printHeader("INSULIN RECORDS FOR " + searchDate);
        cout << setw((termWidth - boxWidth) / 2) << "" << left << setw(12) << "Date" << setw(15) << "Insulin Type"
             << setw(10) << "Time of Day" << right << setw(15) << "Dosage" << endl;
        printBorder('|', '-', '|');

        for (int i = 0; i < insulinEntryCount; i++) {
            if (insulinEntries[i].date == searchDate) {
                found = true;
                cout << setw((termWidth - boxWidth) / 2) << "" << left << setw(12) << insulinEntries[i].date 
                     << setw(15) << insulinEntries[i].insulinType
                     << setw(10) << insulinEntries[i].timeOfDay 
                     << right << setw(15) << insulinEntries[i].dosage << endl;
            }
        }
    ...
    while (1) {
        ...
        if (o == "1") {
            ...
            do {
                ...
                printCenteredLine('|', "3. Search entries by date", '|');
                ...
                case 2:
                    ...
                    break;
                case 3:
                    printHeader("SEARCH BLOOD SUGAR RECORDS BY DATE");
                    printCenteredLine('|', "Enter date to search (YYYY-MM-DD): ", '|');
                    cin >> searchDate;
            ...
            string insulinType, timeOfDay, date, searchDate;
            double dosage;
            int choice;
            ...
            do {
                ...
                printCenteredLine('|', "4. Search entries by date", '|');
                printCenteredLine('|', "5. Return to Main Menu", '|');
                printBorder('|', '-', '|');
                printCenteredLine('|', "Enter your choice (1-5): ", '|');
                cin >> choice;
                ...
                case 4:
                    printHeader("SEARCH INSULIN RECORDS BY DATE");
                    printCenteredLine('|', "Enter date to search (YYYY-MM-DD): ", '|');
                    cin >> searchDate;
                    tracker.searchByDate(searchDate);
                    break;
